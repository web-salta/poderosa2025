...
//JqGrid jqgrid, string sidx, string sord, int page, int rows, 
        [AcceptVerbs("Get", "Post")]
        public JsonResult GetTodoLists_documento(int? ano)  //Gets the todo Lists.
        {
        ...
        public JsonResult mostrar_ano(int anoId)
        {
            var rm = documento_.mostrar_ano(anoId);
            return Json(rm.ano, JsonRequestBehavior.AllowGet);
        }

        [AcceptVerbs("Get", "Post")]
        public JsonResult buscarporano(int ano)  //Gets the todo Lists.        
        {
            using (var db = new ProyectoContext())
            {
               ...
               // int pageIndex = Convert.ToInt32(page) - 1;
                //int pageSize = rows;
                var todoListsResults = db.documento.Select(
                       a => new
                       {
                           a.id,
                           a.anoId,
